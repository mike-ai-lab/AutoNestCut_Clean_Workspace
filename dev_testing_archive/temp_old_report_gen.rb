...
begin
  require_relative 'professional_pdf_generator'
rescue LoadError => e
  puts "WARNING: professional_pdf_generator not available: #{e.message}"
end
...
module AutoNestCut
  class ReportGenerator
    ...
    def generate_report_data(boards, settings = {})
      ...
      total_project_weight = boards.sum { |board| board.total_weight_kg(current_settings['stock_materials'] || {}) }
    ...
    def capture_assembly_data(entity)
      ...
      begin
        ...
        # Add base64 data for HTML embedding
        base64_views = {}
        views.each do |view_name, image_path|
          if File.exist?(image_path)
            image_data = File.binread(image_path)
            base64_data = Base64.strict_encode64(image_data)
            base64_views["#{view_name}_base64"] = base64_data
          end
        end
        views.merge!(base64_views)
    ...
    def generate_interactive_html_content(report_data, boards_data, original_components, hierarchy_tree, settings, timestamp, assembly_data = nil)
      ...
      if assembly_data && assembly_data[:views] && !assembly_data[:views].empty?
        assembly_html = AssemblyExporter.generate_assembly_html_section(
          assembly_data[:views],
          assembly_data[:geometry],
          assembly_data[:entity_name]
        )
        if body_content =~ /(<div[^>]*id="reportContainer"[^>]*>.*?)(<\/div>\s*<\/div>\s*<\/div>\s*<\/div>\s*$)/m
          body_content.sub!(/(<div[^>]*id="reportContainer"[^>]*>.*?)(<\/div>\s*<\/div>\s*<\/div>\s*<\/div>\s*$)/m, "\\1#{assembly_html}\n\\2")
        else
          body_content.sub!(/(<\/div>\s*){3,}$/m, "#{assembly_html}\n</div></div></div>")
        end
      end
...
#{json_data}
    ...
    def calculate_usable_offcuts(boards)
      ...
      boards.each_with_index do |board, index|
        ...
        # Only consider significant offcuts (>0.01 m²)
        if remaining_area > 10000 # 10000 mm² = 0.01 m²
    ...
    def self.generate_scheduled_report(filters, format)
      ...
      when 'json'
        report_data.to_json
      when 'pdf'
        generate_pdf_data(report_data, boards)
      else
        report_data.to_json
      end
    end
    ...
    def self.generate_pdf_data(report_data, boards, assembly_data = nil)
      pdf_generator = ProfessionalPDFGenerator.new
      
      # The professional generator needs a slightly different structure for boards_data
      boards_data = boards.map do |board|
        {
          ...
          efficiency_percentage: board.efficiency_percentage,
          material: board.material,
          parts: board.parts_on_board.map do |part|
            {
              name: part.name,
              width: part.width,
              height: part.height,
      ...
      pdf_generator.generate_professional_pdf(report_data, boards_data, assembly_data)
    end
  end
end
