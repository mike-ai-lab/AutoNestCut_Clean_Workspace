...
module AutoNestCut
  class UIDialogManager
          ...
          unless fits_normal || fits_rotated
            if part_width > stock_width && part_height > stock_height
              errors << {
                ...
                material: material_name,
                issue: 'size',
                part_value: "#{part_width.round(1)}×#{part_height.round(1)}",
                stock_value: "#{stock_width.round(1)}×#{stock_height.round(1)}"
              }
    ...
    def format_validation_message(errors_by_material)
      msg = "⚠️ COMPATIBILITY ISSUES FOUND\n\n"
      ...
      errors_by_material.each do |material, errors|
        ...
        errors.each do |error|
          case error[:issue]
          when 'thickness'
            msg += "  • #{error[:component]}: thickness #{error[:part_value]}mm ≠ sheet #{error[:stock_value]}mm\n"
          when 'width'
            msg += "  • #{error[:component]}: width #{error[:part_value]}mm > sheet #{error[:stock_value]}mm\n"
          when 'height'
            msg += "  • #{error[:component]}: height #{error[:part_value]}mm > sheet #{error[:stock_value]}mm\n"
          when 'size'
            msg += "  • #{error[:component]}: size #{error[:part_value]}mm > sheet #{error[:stock_value]}mm\n"
          end
        end
      end
      ...
      msg += "\n✏️ Update sheet dimensions in the materials list above and try again."
      msg
    end
